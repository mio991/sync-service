use rocket::{get, http::uri::Origin, post, put, routes, Build, Rocket, State};
use std::fmt;
use uuid::Uuid;
...
pub trait StoreApi {
    ...
    fn mount_store_api<'a, B>(self: Self, base: B) -> Self
    where
...
#[get("/<id>")]
async fn retrieve(storage: &State<StorageImpl>, id: Uuid) -> Vec<u8> {
    storage.read(id).await.unwrap_or(Vec::new())
}
